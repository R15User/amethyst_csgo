...
public:
	...
	NETVAR("DT_CSPlayer", "m_iCrosshairId ", cross_id, int);
	...
	{
		return *reinterpret_cast<anim_layer**>(uintptr_t(this) + 0x2980);
	}

	
	int in_cross()
	{
		return *reinterpret_cast<int*>(uintptr_t(this) + 0xb3e8); //cross_id();//;reinterpret_cast<player_t*>(interfaces::entity_list + (cross_id() - 1) * 0x10)->index(); //
	}
	...
	void trans_vec(vec3_t& a, matrix_t& b, vec3_t& out) {
		...
		if (setup_bones(bone_matrix, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, 0.0f)) {
			...
			if (studio_model) {
				...
				if (hitbox) {
					...
					trans_vec(hitbox->mins, bone_matrix[hitbox->bone], min);

					trans_vec(hitbox->maxs, bone_matrix[hitbox->bone], max);
	...
	*/
	...
	bool is_knife_or_nade()
	{
		...
		weapon_t* pWeapon = (weapon_t*)this->active_weapon();

		if (!pWeapon)
			return false;

		std::string WeaponName = pWeapon->get_weapon_data()->m_szWeaponName;
	...
	bool is_pistol()
	{
		...
		if (WeaponName == "weapon_usp_silencer")
			return true;
			...
			return true;
		else if (WeaponName == "weapon_glock")
			return true;
			...
			return true;
		else if (WeaponName == "weapon_cz75a")
			return true;
	...
	bool is_rifle()
	{
		...
		if (WeaponName == "weapon_m4a1")
			...
			return true;
		else if (WeaponName == "weapon_famas")
			return true;
